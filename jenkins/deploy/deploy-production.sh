...
sudo docker pull ditas/privacy-security-evaluator:production

# Get the host IP
HOST_IP="$(ip route get 8.8.8.8 | awk '{print $NF; exit}')"

# Run the docker mapping the ports and passing the host IP via the environmental variable "DOCKER_HOST_IP"
sudo docker run -p 8080:50008 -e DOCKER_HOST_IP=$HOST_IP --restart unless-stopped -d --name privacy-security-evaluator ditas/privacy-security-evaluator:production
ENDSSH